...
def _main(args):
    ...
    with picamera.PiCamera(sensor_mode = 6) as camera:
        ...
        while True:
            ...
            label_result = []
            match_pair = []
            dic = {'person': 'next to another person!', 'motorbike':'doing workout with a Gym Bicycle!', 'bicycle':'doing workout with a Gym Bicycle!', 'sports ball':'doing workout with Yoga Ball!', 'apple':'doing workout with a Yoga Ball!', 'chair':'taking a rest!', 'sofa':'taking a rest!' }
            weights = {'apple':0.15, 'person': 0.8, 'bicycle': 0.7, 'sports ball': 0.2, 'motorbike': 0.35, 'mouse': 0.2, 'chair': 0.65, 'sofa':0.3 }
            ...
            for i in range(1,len(match_pair)):
                try:
                    ...
                    if intersect_area(match_pair[0][1],match_pair[i][1]) > weights[test_class]:
                        if test_class in ['chair', 'sofa'] and body_ratio(match_pair[0][1]) < 1.2:
                        ...
                        if test_class in ['bicycle', 'motorbike']:
                            ...
                            #draw.text(message_origin, message, fill=(255, 0, 0), font=font)
                            break
                        if test_class in ['sports ball', 'apple', 'mouse'] and overall_area(match_pair[i][1]) > 1000:
                            message = "Predicted scenery: Person is " + test_dic
                            print(message_head, test_dic, "\n[INFO] User Pos:", body_pos(match_pair[0][1]))
                            #draw.text(message_origin, message, fill=(255, 0, 0), font=font)
                            break
            ...
            label_result = [] # cache
            if debugging: print("Loop time : %.3f\n" % (time.time() - start_time - 2)) # -2 for the 2 second camera preview time
